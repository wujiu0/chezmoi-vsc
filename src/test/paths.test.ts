...
suite('paths: source → target translation', () => {
  ...
  test('each path segment is decoded independently', () => {
    assert.strictEqual(sourceToTarget('private_dot_ssh/private_config'), '.ssh/config');
  });

  test('nested non-encoded segments pass through', () => {
    assert.strictEqual(sourceToTarget('dot_config/nvim/init.lua'), '.config/nvim/init.lua');
  });
  ...
  test('executable_ prefix is stripped', () => {
    assert.strictEqual(sourceToTarget('executable_dot_local/bin/script.sh'), '.local/bin/script.sh');
  });
