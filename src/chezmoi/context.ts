...
export class ChezmoiContext {
  ...
  async initialize(): Promise<void> {
    ...
    const override = vscode.workspace.getConfiguration('chezmoi').get<string>('sourceDir', '').trim();

    if (override.length > 0) {
      this._sourceDir = override;
    } else {
      const result = await this.cli.exec(['source-path'], { timeout: 5000 });
      this._sourceDir = result.code === 0 && result.stdout.trim().length > 0 ? result.stdout.trim() : undefined;
    }
