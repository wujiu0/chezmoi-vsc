...
export function registerPreview(context: ChezmoiContext, provider: PreviewProvider): vscode.Disposable[] {
  const disposables: vscode.Disposable[] = [];

  disposables.push(vscode.workspace.registerTextDocumentContentProvider(PREVIEW_SCHEME, provider));

  disposables.push(
    vscode.commands.registerCommand('chezmoi-vsc.openPreviewToSide', async (target?: vscode.Uri) => {
      const sourceUri = target instanceof vscode.Uri ? target : vscode.window.activeTextEditor?.document.uri;
      ...
      if (!context.isInsideSource(sourceUri.fsPath)) {
        void vscode.window.showWarningMessage('chezmoi: file is not inside the chezmoi source directory.');
        return;
      }
  ...
  const debounceMs = vscode.workspace.getConfiguration('chezmoi').get<number>('preview.debounce', 300);
  ...
  disposables.push(
    vscode.workspace.onDidOpenTextDocument((doc) => {
      const autoOpen = vscode.workspace.getConfiguration('chezmoi').get<boolean>('preview.autoOpen', false);
